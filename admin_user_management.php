...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_user'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($result_check->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (Name, Email, Password, Role) VALUES (?, ?, ?, 'Passenger')");
                    ...
                    <tbody>
                        <?php while ($user = $result->fetch_assoc()) { ?>
                            <tr>
                                ...
                                <td>
                                    <a href="?delete_id=<?php echo $user['UserID']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure?')">Delete</a>
                                    <button class="btn btn-warning btn-sm" onclick="editUser(<?php echo $user['UserID']; ?>, '<?php echo $user['Name']; ?>', '<?php echo $user['Email']; ?>', '<?php echo $user['Role']; ?>')">Edit</button>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
